...
/**
 * 此类是属性序列的抽象。 所谓的属性序列是指以一定的分隔符分隔的一个属性集合字符串。
 * 这个类和本包中的另一个类的一个方法具有类似的左右，但是这个类因为可以使用规则表达式， 因此功能要强大一些。
 * 
...
public class PropertySerials {
	...
	/**
	 * 构造方法，根据字符串生成一个属性序列类实例。 分隔符使用缺省的'\u0000'。
	 * 
	 * @param source
	 *            属性序列的原始字符串
	 * @since 0.1
	 ...
	 * 
	 * @param splitPattern
	 *            分隔模式，具体的模式规则请参考java.util.regex.Pattern类的说明。
	 * @since 0.1
	 */
