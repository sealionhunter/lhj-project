	...
	public static void main(String[] args) throws IOException {
		...
		File[] fs = f.listFiles(/*new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				...
				return name.endsWith(".html") && !name.startsWith("0index.html");
			}
		}*/);
		for (File ff : fs) {
...
//			f.renameTo(new File("F:/Sealion/books/jpnxd/" + files[i-390]));
			execute2(ff,
					"F:/Sealion/books/jpnxd2/" + ff.getName());
		}
	}
	...
	static void execute2(File from, String end) throws IOException {
		...
		if (from.getName().endsWith(".html") && !from.getName().startsWith("0index.html")) {
...
//		Pattern p = Pattern.compile("<div id=\"content\">.*</div>");
		...
		while ((line = in.readLine()) != null) {
			if (line.contains("<meta name=")) {
				...
				out.newLine();
				continue;
			} else if (line.contains("<script src=\"/qmshu/topfeet2.js\"></script>")) {
				line = line.substring(0, line.indexOf("<script src=\"/qmshu/topfeet2.js\"></script>"));
				// continue;
