...
public class KeyDecodor {
	...
	public static Object decodeObject(String src) throws Exception {
		...
		InflaterOutputStream dos = new InflaterOutputStream(new BufferedOutputStream(baos2), new Inflater());
		decode(bais, dos);
		dos.close();
		bais.close();
	...
	public static String decodeFile(String src) throws Exception {
		byte[] bytes = Base64.getDecoder().decode(src);
		InputStream in = new ByteArrayInputStream(bytes);
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		upzip(in, baos);
	...
	private static void upzip(InputStream in, OutputStream out) throws Exception {
