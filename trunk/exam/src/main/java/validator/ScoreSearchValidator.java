...
public class ScoreSearchValidator implements Validator {

    @SuppressWarnings({ "unchecked", "rawtypes" })
    @Override
    public boolean supports(Class clazz) {
        return clazz.isAssignableFrom(ScoreSearchCommand.class);
    }
    ...
    private void validateRegistInfo(Object target, Errors errors) {
        ScoreSearchCommand v = (ScoreSearchCommand) target;
        ...
        if (v.getPassword() == null || v.getPassword().length() == 0) {
            errors.rejectValue("password", "password", "密码必须填写!");
        }
    }
