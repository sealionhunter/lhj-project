...
public class ApplyDaoImpl implements ApplyDao {
    ...
    public List<Apply> findApplyInfo(SignupDetailSearchCommand condition)
        ...
        if (condition.getDeptId() >= 0) {
            hql += "and D.id=? ";
            paramList.add(condition.getDeptId());
        }
        if (condition.getPostId() >= 0) {
            hql += "and O.id=? ";
            paramList.add(condition.getPostId());
        }
        if (condition.getHomeTown() != null
                && condition.getHomeTown().trim().length() > 0) {
            hql += "and U.homeTown LIKE ? ";
            paramList.add("%" + condition.getHomeTown() + "%");
        }
        if (condition.getPoliticalCode() > 0) {
            hql += "and U.politicalCode=? ";
            paramList.add(condition.getPoliticalCode());
        }
        List<?> list = null;
        if (!paramList.isEmpty()) {
            list = getHibernateTemplate().find(hql, paramList.toArray());
